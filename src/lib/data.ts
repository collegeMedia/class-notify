...
import { Announcement, Assignment, Department, Lecture, Subject, User } from "./types";
...
// Helper function to filter announcements for a specific department
...
// Helper function to get assignments for a specific department
export const getAssignmentsForDepartment = (department: Department): Assignment[] => {
  return assignments
    .filter(assignment => assignment.department === department)
    .sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());
};

// Helper function to get today's lectures for a specific department
export const getTodayLectures = (department: Department): Lecture[] => {
  const today = new Date().toISOString().split('T')[0];
  return lectures
    .filter(lecture => lecture.department === department && lecture.date === today)
    .sort((a, b) => {
      // Sort by start time
...
// Helper function to get subjects for a specific department
export const getSubjectsForDepartment = (department: Department): Subject[] => {
  return subjects
